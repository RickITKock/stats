class Vector<T> {
  constructor(public data: T, public next?: Vector<T>) {}
}

class LinkedList<T> {
  private head?: Vector<T>;
  private tail?: Vector<T>;
  ...
  get(index: number): T | null {
    ...
    let currentVector: Vector<T> = this.head!;
   ...
   */
  push(data: T) {
    const vector: Vector<T> = new Vector<T>(data);
  ...
  pop(): T {
    ...
    const vector: Vector<T> = this.head!;
   ...
   */
  append(data: T) {
    const vector: Vector<T> = new Vector(data);
  ...
  insertAt(index: number, data: T): void {
    ...
    const newVector = new Vector<T>(data);
    let currentVector: Vector<T> | undefined = this.head;
    for (let i = 0; i < index - 1; i++) {
      currentVector = currentVector?.next;
    }

    const nextVector = currentVector?.next;
